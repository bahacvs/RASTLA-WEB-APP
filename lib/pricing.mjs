 ...
 * "gelmedim" bedava, dolayısıyla insanlar üç yere birden yazılıyor ve
 * işletme dolu sandığı saati boş geçiriyor. Tamamını peşin istemek de
 * çözüm değil — dönüşü düşürüyor. Kapora ikisinin arasında: müşteri küçük
 * bir taahhüt veriyor, kalanı tesiste ödüyor.
 *
 * YUKARI yuvarlanıyor. Yön burada müşterinin aleyhine görünüyor ama
 * DEĞİL: kapora toplam borcu değiştirmiyor, yalnızca ne kadarının şimdi
 * ödeneceğini belirliyor — bir lira şimdi, bir lira eksik sonra. Aşağı
 * yuvarlansaydı 1 TL'lik bir hizmette kapora sıfıra düşerdi ve "kapora
 * alındı" diyen bir rezervasyon aslında hiçbir taahhüt taşımazdı.
 *
 * @param {number} total    Ödenecek toplam (indirimler uygulanmış)
 * @param {number|null} percent  Aktivitenin kapora yüzdesi; yoksa null
 * @returns {number} Şimdi tahsil edilecek tutar; kapora yoksa 0
 */
export function depositFor(total, percent) {
  if (!percent || percent <= 0 || total <= 0) return 0;
  // Yüzde 100'ü aşarsa kapora toplamı geçemez: geri kalan eksi olurdu ve
  // "tesiste −50 TL ödenecek" diye bir şey yok.
  return Math.min(total, Math.ceil((total * Math.min(percent, 100)) / 100));
}

/**
 * Rezervasyonun parası nasıl bölünüyor: şimdi ne kadar, tesiste ne kadar.
 *
 * Tek fonksiyon çünkü bu ayrım en az beş yerde soruluyor (ödeme başlatma,
 * hak ediş, iade, bilet ekranı, işletmenin Bugün ekranı) ve her biri kendi
 * `mode === 'deposit' ? … : …` üçlemesini yazsaydı, kapora yüzdesi
 * değiştiğinde ya da yeni bir ödeme kipi eklendiğinde bir tanesi geride
 * kalırdı. Geride kalanın bedeli, tahsil edilenden farklı bir tutarın
 * işletmeye aktarılması olurdu.
 *
 * @param {{ paymentMode: string, totalTRY: number, depositTRY?: number }} booking
 * @returns {{ onlineTRY: number, onsiteTRY: number }}
 */
export function paymentSplit(booking) {
  const total = Math.max(0, Math.trunc(booking.totalTRY));

  if (booking.paymentMode === 'onsite') return { onlineTRY: 0, onsiteTRY: total };

  if (booking.paymentMode === 'deposit') {
    // Kapora KAYITTAN okunuyor, yüzdeden yeniden hesaplanmıyor: işletme
    // yüzdeyi sonradan değiştirebilir ve geçmiş bir rezervasyonun tahsil
    // edilmiş kaporası o değişiklikle birlikte değişemez.
    const deposit = Math.min(total, Math.max(0, Math.trunc(booking.depositTRY ?? 0)));
    return { onlineTRY: deposit, onsiteTRY: total - deposit };
  }

  return { onlineTRY: total, onsiteTRY: 0 };
}
