 ...
 * E-posta gönderimi.
 *
 * SMS ile aynı desen: sağlayıcı anahtarı tanımsızsa **console** kullanılır ve
 ...
 *
 * Dosya düz ESM (`lib/password.mjs`, `lib/db/index.mjs`, `lib/jobs/index.mjs`
 * gibi): uyarı e-postalarını gönderen zamanlanmış iş düğüm betiği olarak da
 * çalışıyor ve TypeScript modülünü doğrudan yükleyemiyor.
 *
 * @typedef {Object} MailMessage
 * @property {string[]} to
 * @property {string} subject
 * @property {string} text  Düz metin. Uyarı e-postaları HTML gerektirmiyor;
 *                          sade metin her yerde okunur.
 *
 * @typedef {{ ok: true, providerRef?: string } | { ok: false, error: string }} MailResult
 *
 * @typedef {Object} MailProvider
 * @property {string} name
 * @property {(message: MailMessage) => Promise<MailResult>} send
 */

/** @type {MailProvider | null} */
let cached = null;
  ...
  return {
    ...
    async send(message) {
      ...
      try {
        ...
        const body = /** @type {{ id?: string }} */ (await response.json());
