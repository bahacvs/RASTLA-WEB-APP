  ...
  async run({ apply = true } = {}) {
    ...
    if (!apply) {
      return {
        job: 'saklama',
        ok: true,
        summary: `${expiredAudit} günlük kaydı ve ${expiredLimits} sayaç satırı silinecek (kuru çalışma).`,
        details: { expiredAudit, expiredLimits, auditDays, limitHours, applied: false },
      };
    }
    ...
    return {
      job: 'saklama',
      ok: true,
      summary: `${auditDeleted} günlük kaydı, ${limitsDeleted} sayaç satırı silindi.`,
      details: { auditDeleted, limitsDeleted, auditDays, limitHours, applied: true },
    };
  },
};
