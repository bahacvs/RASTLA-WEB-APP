...
export async function expireBooking(bookingId) {
  ...
  const booking = await client.get('SELECT slot_id, units FROM bookings WHERE id = ?', [bookingId]);
  ...
  if (booking.slot_id) await releaseCapacity(booking.slot_id, booking.units);
  return true;
}
